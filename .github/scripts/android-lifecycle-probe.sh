...
printf '%s\n' "${REAL_MODEL_PROBE}" | grep -Eq '^web_fetch_bytes=[1-9][0-9]*adb shell run-as "${PACKAGE}" rm -f \
  files/ntd97-device-evidence.nde97 \
  files/ntd97-device-evidence.txt || true
adb shell am start -W -n "${PHYSICAL_ACTIVITY}" >/dev/null
...
printf '%s\n' "${REAL_MODEL_PROBE}" | grep -Eq '^web_fetch_redirects=[0-5]adb shell run-as "${PACKAGE}" rm -f \
  files/ntd97-device-evidence.nde97 \
  files/ntd97-device-evidence.txt || true
adb shell am start -W -n "${PHYSICAL_ACTIVITY}" >/dev/null
