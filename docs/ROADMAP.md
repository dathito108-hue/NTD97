...
- paired aliases are create-only until explicit revoke, including race-safe no-replacement commit semantics; staged bytes are synced, committed in-directory, reloaded through the production validator and only then reported successful;
