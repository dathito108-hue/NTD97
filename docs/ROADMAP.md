...
- optional search credentials use an explicit bounded HTTP header name/value, reject reserved transport headers/control characters and are attached only by the WebSearch-specific GET transport; the secret value is AES-GCM encrypted with an Android Keystore key before app-private persistence, while generic `web.fetch` and upload transports never see it;
- decrypted WebSearch credentials remain private to `NtdWebPlatform` and are never re-rendered into the settings UI; saving the same credential header with a blank secret preserves the existing encrypted credential, while clearing the header explicitly removes it;
...
- emulator acceptance uses a non-secret debug credential to prove Keystore-backed ciphertext persistence, header delivery and cross-origin redirect rejection, then performs a real OpenSearch discovery/search before restoring the Generic JSON profile for the production ActionFabric probe;
